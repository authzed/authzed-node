import { CallOptions, Client, ClientReadableStream, ClientUnaryCall, Metadata, ServiceError } from '@grpc/grpc-js';

/**
 * Picks all function properties of a type that except for those with a specific return type
 */
export type ExcludeReturnType<T, U> = 
  Pick<T, { [K in keyof T]: T[K] extends (_: infer _T1) => U ? never : K }[keyof T]>;

/**
 * Picks all function properties of a type that have the specific return type
 */
export type PickReturnType<T, U> = 
  Pick<T, { [K in keyof T]: T[K] extends (_: infer _T2) => U ? K : never }[keyof T]>;
...
export type PromisifiedCallbackClient<C> = {
  [prop in Exclude<keyof ExcludeReturnType<C, ClientReadableStream<unknown>>, keyof Client>]:
    C[prop] extends UnaryCall<infer T, infer U> ? PromisifiedCall<T, U>
    : never
}
...
export type PromisifiedStreamClient<C> = {
  [prop in Exclude<keyof PickReturnType<C, ClientReadableStream<unknown>>, keyof Client>]:
  C[prop] extends StreamCall<infer T1, infer U1> ? PromisifiedCall<T1, U1[]>
    : never
}
 ...
 */
export type PromisifiedClient<C> = PromisifiedCallbackClient<C> & PromisifiedStreamClient<C>