import authzedv0 from "./v0";
import authzedv1 from "./v1alpha1";
...
`);
    ...
    v1client.writeSchema(writeSchemaRequest, function (err, response) {
      ...
      const v0client = authzedv0.NewClient(
        "sometoken",
        "localhost:50051",
        true
      );
      ...
      v0client.write(writeRequest, function (err, response) {
        ...
        v0client.check(checkRequest, function (err, response) {
          expect(err).toBe(null);
          expect(response.getMembership()).toBe(
            authzedv0.CheckResponse.Membership.MEMBER
          );
          done();
        });
      });
